...
def call_openai(
    ...
    return resp.choices[0].message.content, end - start
...
def compare_models(prompt: str) -> dict:
    ...
    mini_cost = (len(mini_response.split()) / 0.75) / 1000 * PRICING_PER_1K_TOKENS["gpt-4o-mini"]["output"]
...
def batch_compare(prompts: list[str]) -> list[dict]:
    ...
    """
    return [
        {**compare_models(prompt), "prompt": prompt}
